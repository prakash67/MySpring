...
public class AdmissionFormController {
	 ...
	 * 
	 * if(!(name.isEmpty()) && age != 0){ modelAndView.addObject("msg","I am "+ name
	 * + " and I am "+ age +" years old"); }else{ if(name.isEmpty()){
	...
	public void initBinder(WebDataBinder binder) throws ParseException {
		...
		binder.registerCustomEditor(Date.class, "applicantDob", new CustomDateEditor(dateFormat, true));
	...
	public ModelAndView admissionSubmission(@Valid @ModelAttribute("admission") AdmissionBean admissionBean, BindingResult result) {
		 ...
		 * admissionBean.setApplicantAge(applicantAge); ModelAndView modelAndView = new
